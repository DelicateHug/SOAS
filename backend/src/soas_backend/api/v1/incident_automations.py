"""Incident automation views and manual run endpoints."""
...
router = APIRouter(tags=["incident-automations"])
...
@router.get("/incidents/{incident_id}/automations")
async def list_incident_automations(
    incident_id: UUID,
    _: dict = Depends(require_permission("incident", "read")),
    db: AsyncSession = Depends(get_db),
):
    """List automation executions for this incident, grouped by automation."""
    result = await db.execute(
        select(ExecutionLog)
        .where(ExecutionLog.incident_id == incident_id)
...
@router.post("/incidents/{incident_id}/automations/{execution_id}/evidence")
async def toggle_execution_evidence(
    ...
    """Toggle evidence status on an automation execution."""
    result = await db.execute(
        select(ExecutionLog)
        .where(ExecutionLog.id == execution_id, ExecutionLog.incident_id == incident_id)
        .options(selectinload(ExecutionLog.automation))
    )
...
@router.post("/incidents/{incident_id}/automations/{automation_id}/run", status_code=status.HTTP_202_ACCEPTED)
async def run_automation_on_incident(
    incident_id: UUID,
    automation_id: UUID,
    current_user: User = Depends(get_current_user),
    ...
    """Manually trigger an automation in this incident's context."""
    ...
    # Ensure incident data is cached in Redis so the automation bridge
    # can resolve get_incident_var() calls at runtime.
    ...
    incident_data = {
        ...
        "metadata": metadata,
        **metadata,  # Flatten custom vars as top-level keys
    }
    await cache.cache_incident(incident_id, incident_data)
    ...
    task = celery.send_task(
        ...
        args=[
            str(execution.id),
            str(automation.id),
            {"manual_run": True, "incident_id": str(incident_id)},
        ],
    )
...
async def get_incident_evidence(
    ...
    notes_result = await db.execute(
        select(IncidentNote)
        .where(IncidentNote.incident_id == incident_id, IncidentNote.is_evidence == True)
        .options(selectinload(IncidentNote.creator))
    ...
    files_result = await db.execute(
        select(IncidentFile)
        .where(IncidentFile.incident_id == incident_id, IncidentFile.is_evidence == True)
    ...
    timeline_result = await db.execute(
        ...
        .where(TimelineEntry.incident_id == incident_id, TimelineEntry.is_evidence == True)
    ...
    exec_result = await db.execute(
        select(ExecutionLog)
        .where(ExecutionLog.incident_id == incident_id, ExecutionLog.is_evidence == True)  # noqa: E712
