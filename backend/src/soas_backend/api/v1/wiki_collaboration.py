...
# ---------------------------------------------------------------------------
# DB helpers for Y.js state persistence
# ---------------------------------------------------------------------------
...
async def _read_from_client(
    ...
    presence_key = f"wiki-collab:{page_id}:presence"
    ...
    while True:
            ...
            if now - last_save_time > 5:
                ...
                try:
                    ...
                    if state_b64:
                        ...
                        await _save_yjs_state(page_id, state_bytes)
                except Exception:
                    pass
            continue
        ...
        if msg_type == "save_html":
            # Client sends rendered HTML for DB persistence
            if not can_edit:
                continue
            html = msg.get("html", "")
            try:
                await _save_html_content(page_id, html, user_id)
            except Exception:
                pass
            continue
...
@wiki_collab_ws_router.websocket("/api/v1/ws/wiki/{page_id}")
async def wiki_collaboration_ws(websocket: WebSocket, page_id: str):
    ...
    can_edit = "wiki:update" in permissions
    ...
    try:
        ...
        await _update_editor_count(page_id, 1)
        ...
        # Load existing Y.js state from DB
        yjs_state = await _load_yjs_state(page_id)
        ...
        await asyncio.gather(
            _read_from_client(websocket, r, channel, user_id, page_id, can_edit, display_name),
