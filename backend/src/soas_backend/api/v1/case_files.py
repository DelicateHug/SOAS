"""Case file upload/download endpoints."""
...
from soas_backend.services.case_file_service import CaseFileService
from soas_shared.schemas.case_file import CaseFileRead
...
@router.get("/cases/{case_id}/files", response_model=list[CaseFileRead])
async def list_case_files(
    case_id: UUID,
    current_user: User = Depends(get_current_user),
    payload: dict = Depends(require_permission("case_file", "read")),
    db: AsyncSession = Depends(get_db),
):
    svc = CaseFileService(db)
    files = await svc.list_files(case_id)
...
async def upload_case_file(
    ...
    current_user: User = Depends(get_current_user),
    payload: dict = Depends(require_permission("case_file", "upload")),
    db: AsyncSession = Depends(get_db),
):
...
@router.get("/cases/{case_id}/files/{file_id}/download")
async def download_case_file(
    case_id: UUID,
    file_id: UUID,
    _: dict = Depends(require_permission("case_file", "read")),
    db: AsyncSession = Depends(get_db),
):
    svc = CaseFileService(db)
...
async def delete_case_file(
    ...
    file_id: UUID,
    current_user: User = Depends(get_current_user),
    payload: dict = Depends(require_permission("case_file", "delete")),
    db: AsyncSession = Depends(get_db),
):
    svc = CaseFileService(db)
    record = await svc.get(file_id)
...
@router.post("/cases/{case_id}/files/{file_id}/evidence", response_model=CaseFileRead)
