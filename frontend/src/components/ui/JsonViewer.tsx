...
function JsonNode({ value, depth = 0 }: { value: unknown; depth?: number }) {
  ...
  if (value === null) return <span className="text-[hsl(var(--muted-foreground))]">null</span>;
  ...
  if (Array.isArray(value)) {
    ...
    if (collapsed) {
      return (
        <button onClick={() => setCollapsed(false)} className="text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))]">
          [{value.length} items]
        </button>
      );
    }
    return (
      <span>
        <button onClick={() => setCollapsed(true)} className="text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))]">[</button>
        <div className="pl-4">
          {value.map((item, i) => (
            <div key={i}>
  ...
  return (
    <div className={cn("rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))]", className)}>
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-[hsl(var(--border))]">
        <span className="text-[10px] uppercase tracking-wider text-[hsl(var(--muted-foreground))] font-medium">JSON</span>
        <button
          onClick={() => setRaw(!raw)}
          className="text-[10px] px-1.5 py-0.5 rounded text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted))]"
        >
          {raw ? "Tree" : "Raw"}
        </button>
      </div>
