...
export function FormsTab({ incidentId }: Props) {
  const queryClient = useQueryClient();
  const [showSubmitForm, setShowSubmitForm] = useState(false);
  const [selectedFormId, setSelectedFormId] = useState<string>("");
  const [formData, setFormData] = useState<Record<string, unknown>>({});

  // Fetch submissions for this incident
  const { data: submissions } = useQuery({
    queryKey: ["incident-form-submissions", incidentId],
    queryFn: () =>
      api.get<FormSubmission[]>(`/incidents/${incidentId}/form-submissions`),
  });

  // Fetch active form definitions for the dropdown
  ...
  return (
    <div className="space-y-4">
      <SectionHeader
        ...
        action={
          !showSubmitForm ? (
            <WriteGuard blockedTitle="Start work on this incident to submit forms">
              <button
